...
const features = [
  ...
  },
  {
    ...
    subtitle: 'EDGE SPEED',
    detail: 'Served on Cloudflare V8 global edge nodes with sub-50ms loading speed and 99.99% uptime guarantee.',
    type: 'growth',
...
function FeaturePreview({ type }) {
  ...
  if (type === 'action') {
    return (
      <div className="w-full p-5 rounded-2xl bg-slate-50/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-slate-200/90 dark:border-zinc-700/80 shadow-lg space-y-3 text-left">
        <div className="flex items-center justify-between text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
          ...
          <span className="text-slate-400 font-mono text-[9px] px-1.5 py-0.5 rounded bg-slate-200/60 dark:bg-zinc-800">LIVE</span>
        </div>
        <div className="space-y-2 text-xs font-medium text-slate-800 dark:text-slate-200">
          <div className="flex items-center gap-2.5">
            <span className="w-4 h-4 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-[10px] font-bold shrink-0">✓</span>
  ...
  if (type === 'connect') {
    return (
      <div className="w-full p-5 rounded-2xl bg-slate-50/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-slate-200/90 dark:border-zinc-700/80 shadow-lg space-y-3.5 text-center">
        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest block">
          OFFICIAL INTEGRATIONS
        </span>
        <div className="flex items-center justify-center gap-3">
          {/* Instagram */}
          <span className="w-9 h-9 rounded-xl bg-gradient-to-tr from-amber-500 via-rose-500 to-purple-600 text-white flex items-center justify-center shadow-md transition-transform hover:scale-110">
          ...
          {/* LinkedIn */}
          <span className="w-9 h-9 rounded-xl bg-[#0A66C2] text-white flex items-center justify-center shadow-md transition-transform hover:scale-110">
            <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
          ...
          {/* Google */}
          <span className="w-9 h-9 rounded-xl bg-white border border-slate-200 shadow-md flex items-center justify-center transition-transform hover:scale-110">
          ...
          {/* GitHub */}
          <span className="w-9 h-9 rounded-xl bg-[#24292F] text-white flex items-center justify-center shadow-md transition-transform hover:scale-110">
            <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
  ...
  return (
    <div className="w-full p-5 rounded-2xl bg-slate-50/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-slate-200/90 dark:border-zinc-700/80 shadow-lg space-y-2.5 text-left">
      <div className="flex items-center justify-between text-xs font-bold">
        <span className="text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[10px]">EDGE LATENCY</span>
      ...
      <div className="h-2 w-full bg-slate-200 dark:bg-zinc-800 rounded-full overflow-hidden">
        <div className="h-full w-11/12 bg-emerald-500 rounded-full shadow-xs" />
      </div>
    </div>
  );
}
...
export default function WhySection() {
  return (
    <section id="why" className="w-full max-w-7xl mx-auto py-16 px-4 sm:px-6">
      ...
      {/* Modern High-Contrast Glassy Cards */}
      <ScrollStack
      ...
      >
        {features.map((feature) => (
          <ScrollStackItem
            key={feature.number}
            itemClassName={`bg-white dark:bg-zinc-900/95 border ${feature.border} shadow-2xl shadow-emerald-500/5 transition-all duration-300 hover:shadow-emerald-500/10`}
          >
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center h-full">
              ...
              <div className="md:col-span-7 space-y-4">
                <div className="flex items-center gap-3">
                  <span className="text-4xl sm:text-5xl font-black font-serif text-emerald-500 dark:text-emerald-400">
                    {feature.number}.
                  </span>
                  <span className="text-[10px] font-bold tracking-wider text-emerald-600 dark:text-emerald-400 uppercase bg-emerald-500/10 dark:bg-emerald-500/20 px-3.5 py-1 rounded-full border border-emerald-500/30">
                    {feature.subtitle}
                  </span>
                </div>
              ...
              {/* Right Column: High-Contrast Glassy Preview Widget */}
